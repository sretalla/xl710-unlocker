...
int main(int argc, const char **argv) {
  ...
  const char *ethDev = "eth3"; // "eth3" = If card is on eth3
  /* Find a record like this:
    ...
    00006870 + 0b => 0003 
   */
  int phy0_offset = 0x6957; // 0x6870 on my card; find it with mytool
  
  int offset = 2*(phy0_offset + 0x0); /* PHY0 + MISC0(register with 11bit) */
  int length = 2;
  int mod    = 0;
  ...
  if (!eeprom) die("calloc");
    ...
    eeprom->len    = length;
    eeprom->offset = offset + 0x0; /* offset between 000b values */
    
    /* Remove bit 0800 = qualification from whatever was in register 8 */
    *(uint16_t*)(eeprom+1) = 0x620c; // 0x230c = 0x2b0c - 0x8000
